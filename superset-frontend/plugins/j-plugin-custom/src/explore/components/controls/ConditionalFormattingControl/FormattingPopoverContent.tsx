...
const renderOperatorFieldsWithName = ({
  getFieldValue,
  name,
}: GetFieldValue) =>
  isOperatorNone(getFieldValue('operator')) ? (
    ...
    <Row gutter={12}>
      <Col span={9}>
        <FormItem
          name={[name, 'targetValueLeft']}
          label={t('Left value')}
          rules={rulesTargetValueLeft}
          dependencies={[name, ...targetValueLeftDeps]}
          validateTrigger="onBlur"
          trigger="onBlur"
        >
          <FullWidthInputNumber />
      ...
      <Col span={9}>
        <FormItem
          name={[name, 'targetValueRight']}
          label={t('Right value')}
          rules={rulesTargetValueRight}
          dependencies={[name, ...targetValueRightDeps]}
          validateTrigger="onBlur"
          trigger="onBlur"
        >
          <FullWidthInputNumber />
  ...
  return (
    ...
    >
      <Tabs centered>
        ...
        {tabs?.map((tab, i) => (
          <Tabs.TabPane tab={tab} key={i}>
            <FormItem name="formatType" initialValue={tab} hidden={true} />
            <Row gutter={12}>
              <Col span={12}>
                <FormItem
                  name={[tab, 'column']}
            ...
            <FormItem noStyle shouldUpdate={shouldFormItemUpdate} name={tab}>
              {renderOperatorFieldsWithName}
            </FormItem>
          </Tabs.TabPane>
        ))}
      </Tabs>
